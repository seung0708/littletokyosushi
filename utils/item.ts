...
export const calculateItemTotal = (item: OrderItem) => {
    const modifierTotal: number | undefined = item?.order_item_modifiers?.reduce((total: number, modifier: OrderItemModifier) => {
        return total + modifier.order_item_modifier_options?.reduce((optTotal: number, opt: OrderItemModifierOption) => 
            optTotal + opt.option_price, 0
        );
    }, 0);
...
export const calculateTotalPrice = (basePrice: CartItem['base_price'], quantity: CartItem['quantity'], modifiers: CartItemModifier[]) => {
    const modifierPrice = modifiers.reduce((total: number, modifier: CartItemModifier) => {
        return total + (modifier?.cart_item_modifier_options ?? []).reduce((optTotal: number, opt: CartItemModifierOption) => optTotal + (opt?.modifier_option_price ?? 0), 0);
    }, 0);
